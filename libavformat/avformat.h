...
#define PKT_FLAG_TAINTED	0x0002
...
/**
 * Interruptible version of av_find_stream_info.
 * If cb returns a value != 0, then the probe loop will get interrupted,
 * whatever happens (eg. to cancel probing after a timeout, or whatever).
 *
 * @param ic media file handle
 * @param cb cancellation callback. return 0 to carry on probing normally, > 0 otherwise.
 * @param userdata data to be passed to cb during call
 * @return >=0 if OK. AVERROR_xxx if error.
 */
int av_find_stream_info2(AVFormatContext *ic, int (*cb)(void *), void *userdata);
...
int64_t av_gen_search2(AVFormatContext *s, int stream_index, int64_t target_ts, int64_t pos_min, int64_t pos_max, int64_t pos_limit, int64_t ts_min, int64_t ts_max, int flags, int64_t *ts_ret, int64_t (*read_timestamp)(struct AVFormatContext *, int , int64_t *, int64_t ), int (* cb)(void *), void * userdata);
