...
static int mov_realloc_extradata(AVCodecContext *par, MOVAtom atom)
{
    int err = 0;
...
static int64_t mov_read_atom_into_extradata(MOVContext *c, AVIOContext *pb, MOVAtom atom,
                                        AVCodecContext *par, uint8_t *buf)
{
...
static int mov_read_targa_y216(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!ret && c->fc->nb_streams >= 1) {
        AVCodecContext *par = c->fc->streams[c->fc->nb_streams-1]->codec;
        ...
        if (atom.size == 16) {
            ...
            if (!ret) {
                ...
                if (length == atom.size) {
                    ...
                    av_log(c, AV_LOG_DEBUG, "color_range: %d\n", par->color_range);
                } else {
...
static int mov_read_wave(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (st->codec->codec_id == AV_CODEC_ID_QDM2 ||
        ...
        ret = ff_get_extradata(c->fc, st->codec, pb, atom.size);
        if (ret < 0)
            return ret;
...
static int mov_read_glbl(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (ret < 0)
        return ret;
    if (atom.type == MKTAG('h','v','c','C') && st->codec->codec_tag == MKTAG('d','v','h','1'))
...
static int mov_read_dvc1(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    av_freep(&st->codec->extradata);
    ret = ff_get_extradata(c->fc, st->codec, pb, atom.size - 7);
    if (ret < 0)
        return ret;
...
static int mov_read_strf(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    avio_skip(pb, 40);
    av_freep(&st->codec->extradata);
    ret = ff_get_extradata(c->fc, st->codec, pb, atom.size - 40);
    if (ret < 0)
        return ret;
                                ...
                                int64_t size)
{
    ...
    if (st->codec->codec_tag == MKTAG('t','m','c','d')) {
        ...
        ret = ff_get_extradata(c->fc, st->codec, pb, size);
        if (ret < 0)
            return ret;
        if (size > 16) {
            ...
            st->avg_frame_rate.num = AV_RB32(st->codec->extradata + 8); /* timescale */
            st->avg_frame_rate.den = AV_RB32(st->codec->extradata + 12); /* frameDuration */
#if FF_API_LAVF_AVCTX
FF_DISABLE_DEPRECATION_WARNINGS
...
int ff_mov_read_stsd_entries(MOVContext *c, AVIOContext *pb, int entries)
{
    ...
    for (pseudo_stream_id = 0;
        ...
        av_log(c->fc, AV_LOG_TRACE,
               ...
               av_fourcc2str(format), st->codec->codec_type);
        ...
        if (sc->extradata && st->codec->extradata) {
            int extra_size = st->codec->extradata_size;
            ...
            if (!sc->extradata[pseudo_stream_id])
                return AVERROR(ENOMEM);
            memcpy(sc->extradata[pseudo_stream_id], st->codec->extradata, extra_size);
            av_freep(&st->codec->extradata);
            st->codec->extradata_size = 0;
        }
        sc->stsd_count++;
...
static int mov_read_stsd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    av_freep(&st->codec->extradata);
    st->codec->extradata_size = sc->extradata_size[0];
    if (sc->extradata_size[0]) {
        st->codec->extradata = av_mallocz(sc->extradata_size[0] + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!st->codec->extradata)
            return AVERROR(ENOMEM);
        memcpy(st->codec->extradata, sc->extradata[0], sc->extradata_size[0]);
    }
    ...
    while (get_edit_list_entry(mov, msc, edit_list_index, &edit_list_media_time,
        ...
        for (; current < e_old_end; current++, index++) {
            ...
            if (curr_cts < edit_list_media_time || curr_cts >= (edit_list_duration + edit_list_media_time)) {
                if (st->codec->codec_type == AVMEDIA_TYPE_AUDIO && st->codec->codec_id != AV_CODEC_ID_VORBIS &&
                    ...
                    if (edit_list_start_encountered == 0) {
                        ...
                        if (st->codec->codec_type == AVMEDIA_TYPE_AUDIO &&
                            first_non_zero_audio_edit > 0 && st->codec->codec_id != AV_CODEC_ID_VORBIS) {
            ...
            if ((curr_cts + frame_duration >= (edit_list_duration + edit_list_media_time)) &&
                ...
                if (ctts_data_old) {
                    ...
                    if (st->codec->codec_type != AVMEDIA_TYPE_AUDIO && found_keyframe_after_edit == 0) {
...
static void mov_build_index(MOVContext *mov, AVStream *st)
{
    ...
    if (sc->elst_count) {
        ...
        if (!multiple_edits && !mov->advanced_editlist &&
            st->codec->codec_id == AV_CODEC_ID_AAC && start_time > 0)
    ...
    if (st->start_time == AV_NOPTS_VALUE && st->codec->codec_type == AVMEDIA_TYPE_VIDEO && st->nb_index_entries > 0) {
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (sc->dref_id-1 < sc->drefs_count && sc->drefs[sc->dref_id-1].path) {
        ...
        if (c->enable_drefs) {
            if (mov_open_dref(c, &sc->pb, c->fc->filename, dref, &c->fc->interrupt_callback) < 0)
                av_log(c->fc, AV_LOG_ERROR,
...
static int mov_read_elst(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (atom.size != edit_count * elst_entry_size) {
        av_log(c->fc, AV_LOG_WARNING, "Invalid edit list entry_count: %d for elst atom of size: %"PRId64" bytes.\n",
    ...
    for (i = 0; i < edit_count && atom.size > 0 && !pb->eof_reached; i++) {
        ...
        if (e->time < 0 && e->time != -1) {
            av_log(c->fc, AV_LOG_WARNING, "Track %d, edit %d: Invalid edit list media time=%"PRId64"\n",
...
static int mov_read_av1c(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    ret = ff_get_extradata(c->fc, st->codec, pb, atom.size - 4);
    if (ret < 0)
        return ret;
...
static int mov_read_vpcc(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    st->codec->color_range     = (color_range & 1) ? AVCOL_RANGE_JPEG : AVCOL_RANGE_MPEG;
    st->codec->color_primaries = color_primaries;
    st->codec->color_trc       = color_trc;
...
static int mov_read_dmlp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    st->codec->frame_size = 40 << (ratebits & 0x7);
    st->codec->sample_rate = mlp_samplerate(ratebits);
    st->codec->channels = truehd_channels(channel_assignment);
    st->codec->channel_layout = truehd_layout(channel_assignment);
...
#endif
        if (st->codec->codec_id == AV_CODEC_ID_MP3 && !st->need_parsing && pkt->size > 4) {
